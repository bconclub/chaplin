...
async function ensureReferenceAssets(admin: ReturnType<typeof getSupabaseAdminClient>, referenceImages: string[]) {
  const result = await admin.from("media_assets").select("id").in("id", referenceImages);
...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json({ error: message }, { status: /Sign in|access|not found/i.test(message) ? 403 : 400 });
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const { admin } = await ownedProduct(request, id);
    const product = ProductCardSchema.parse(await request.json());
    await ensureReferenceAssets(admin, product.reference_images);
