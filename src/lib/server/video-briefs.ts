...
export async function createVideoBrief(input: VideoBriefInputData, ownerId: string) {
  ...
  try {
    ...
    const shots = plan.map((shot) => {
      ...
      return {
        video_brief_id: briefId, shot_number: shot.shotNumber, duration_seconds: 5, beat: shot.beat,
        ...
        image_prompt: productInput ? composeProductImagePrompt(productInput) : actor ? composeImagePrompt(actor, promptShot) : null,
        video_prompt: productInput ? composeProductVideoPrompt(productInput) : actor ? composeVideoPrompt(actor, promptShot) : null,
